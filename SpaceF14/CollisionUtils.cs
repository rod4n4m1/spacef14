...
namespace SpaceF14
{
	...
	public static class CollisionUtils
	{
		...
		/// <param name="draw1">Draw1.</param>
		/// <param name="data1">Data1.</param>
		/// <param name="draw2">Draw2.</param>
		/// <param name="data2">Data2.</param>
		/// <param name="collidedPoint">Collided point.</param>
		public static bool DetectCollision(Rectangle draw1, Color[] data1, Rectangle draw2, Color[] data2, ref Point collidedPoint)
		{
			bool collided = false;
			if (draw1.Intersects(draw2))
			{
				// Find the bounds of the rectangle intersection
				int top = Math.Max(draw1.Top, draw2.Top);
				int bottom = Math.Min(draw1.Bottom, draw2.Bottom);
				int left = Math.Max(draw1.Left, draw2.Left);
				int right = Math.Min(draw1.Right, draw2.Right);
				...
				for (int y = top; y < bottom; y++)
				{
					for (int x = left; x < right; x++)
					{
						...
						Color color1 = data1[(x - draw1.Left) + (y - draw1.Top) * draw1.Width];
						Color color2 = data2[(x - draw2.Left) + (y - draw2.Top) * draw2.Width];

						// If both pixels are not completely transparent,
						if (color1.A != 0 && color2.A != 0)
						{
							// then an pixel collision has been found
							collided = true;
							collidedPoint.X = x;
							collidedPoint.Y = y;
						}
